...
def predicted_neg_loglike(x, raw_output):
    ...
    M = reconstruct_sym_matrix(flat_inv_cov, dim)  # (batch_size, dim, dim)
    diff = x - x0
...
class MCMCPlanck:
    def __init__(self, initial_point, T, model_file, param_scaler_file, target_scaler_file, sigma):
        ...
        # Load TFLite model for sampling
        ...
        # Load the full Keras model and create a submodel that outputs the raw parameters.
        # The final model has a Lambda layer and an InverseScalingLayer, so we use the Dense layer
        # that produced raw_output. We assume this is full_model.layers[-3].
        full_model = tf.keras.models.load_model(
            model_file,
            custom_objects={'InverseScalingLayer': InverseScalingLayer, 'ScaledTanh': ScaledTanh},
            compile=False
        )
        self.raw_model = tf.keras.models.Model(inputs=full_model.input, outputs=full_model.layers[-3].output)
        ...
        self.accepted = 0
        self.total_steps = 0
        self.eigenvalues_history = []  # Record (step, eigenvalues) tuples
    ...
    def compute_eigenvalues(self, point):
        """
        Compute the eigenvalues of the inverse covariance matrix produced by the full model raw output.
        """
        # Scale the input point using the parameter scaler.
        point_scaled = self.param_scaler.transform(point.reshape(1, -1)).astype(np.float32)
        # Use the raw submodel to obtain the raw output vector.
        raw_output = self.raw_model.predict(point_scaled)
        raw_output = np.squeeze(raw_output)
        # Expected raw_output structure: [offset, x0_1, ..., x0_dim, flat_inv_cov...]
        flat_inv_cov = raw_output[1 + self.dim:]  # extract the inverse covariance elements
        M = np.zeros((self.dim, self.dim), dtype=np.float32)
        k = 0
        for i in range(self.dim):
            for j in range(i, self.dim):
                M[i, j] = flat_inv_cov[k]
                M[j, i] = flat_inv_cov[k]
                k += 1
    ...
    def chain(self, n_steps, burn_in=1000, adaptation_interval=50, target_acceptance=0.234, eigenvalue_interval=None):
        """
        Run the MCMC chain. If eigenvalue_interval is specified (e.g. every 100 steps),
        record the eigenvalues of the inverse covariance matrix at those steps.
        """
        if burn_in > 0:
            ...
            for block in range(n_blocks):
                ...
                for _ in range(adaptation_interval):
                    accepted = self.step()
                    if eigenvalue_interval is not None and (self.total_steps % eigenvalue_interval == 0):
                        eig_vals = self.compute_eigenvalues(self.current_point)
                        self.eigenvalues_history.append((self.total_steps, eig_vals))
                    if accepted:
                        accepted_in_block += 1
...
# ----------------------------
# Main Execution: Run MCMC and Produce Corner Plot & Eigenvalue Evolution Plot
# ----------------------------
if __name__ == "__main__":
    ...
    T = 5
    model_file = os.path.join("n_dim_models", f"6d_model.h5")
    param_scaler_file = os.path.join("data", f"6d_param_scaler.pkl")
    target_scaler_file = os.path.join("data", f"6d_target_scaler.pkl")
    ...
    # Run MCMC with eigenvalue recording every 100 steps (user can adjust eigenvalue_interval)
    eigenvalue_interval = 100
    sampler.chain(n_steps=25000, burn_in=1000, adaptation_interval=50, target_acceptance=0.25, eigenvalue_interval=eigenvalue_interval)
    ...
    plt.grid(True)
    plt.savefig("6d_loglkl_evolution.png", dpi=300)
    plt.show()
    
    # ----------------------------
    # Plot Eigenvalue Evolution
    # ----------------------------
    if sampler.eigenvalues_history:
        steps, eigenvalues_list = zip(*sampler.eigenvalues_history)
        steps = np.array(steps)
        eigenvalues_array = np.array(eigenvalues_list)  # shape: (num_records, dim)
        dim = sampler.dim
        # Arrange subplots (e.g. 2 rows x 3 columns for 6d)
        ncols = 3 if dim > 3 else dim
        nrows = int(np.ceil(dim / ncols))
        fig, axes = plt.subplots(nrows, ncols, figsize=(5*ncols, 4*nrows))
        axes = np.array(axes).flatten()
        for i in range(dim):
            axes[i].plot(steps, eigenvalues_array[:, i], lw=1.5)
        ...
        for j in range(dim, len(axes)):
            axes[j].axis('off')
        plt.tight_layout()
        plt.savefig("eigenvalues_vs_steps.png", dpi=300)
        plt.show()
    
    # ----------------------------
    # Corner Plot (remaining code remains unchanged)
    # ----------------------------
    ...
    corner_plot(mcmc_data, training_data, training_weights, bestfit, labels_27, dims_to_plot, output_filename="6d_corner_plot.png")
