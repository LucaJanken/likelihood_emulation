...
# ----------------------------
dim = 27  # Change this to the desired number of parameters
...
# ----------------------------
with open(os.path.join(data_dir, "planck_param_scaler.pkl"), "rb") as f:
    scaler_X = pickle.load(f)
with open(os.path.join(data_dir, "planck_target_scaler.pkl"), "rb") as f:
    scaler_y = pickle.load(f)

model_path = os.path.join(model_dir, "planck_model.h5")
...
lines_per_file_eval = 50000  # Maximum number of rows to load per file for evaluation
burn_in_lines = 500         # Number of lines to skip in each chain file
skip_training_lines = 50000 + burn_in_lines  # Skip training and burn-in lines
...
neg_loglike = combined_chain[:, 1]  # True -loglike values
...
y_true = sampled_chain[:, 1]        # shape: (num_samples,)
...
plt.title("Planck Model Prediction Residuals in $\chi^2$")
plt.grid(True)
plot_path = os.path.join(plots_dir, "planck_model_residuals.png")
