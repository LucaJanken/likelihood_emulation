...
# ---------------------------
# MPI Initialization for Parallel Tuning
# ---------------------------
...
# Optionally set a unique seed per process.
...
# ---------------------------
# MPI-Parallel Hyperparameter Tuning using Keras Tuner (Random Search)
# ---------------------------
total_trials = 2500
trials_per_proc = total_trials // size
if rank == size - 1:
    trials_per_proc += total_trials % size
...
tuner = kt.RandomSearch(
    build_model,
    objective="val_loss",
    max_trials=trials_per_proc,  # Each MPI process handles a subset.
    directory="random_search_results",
    project_name="chain_hps_6D_random_search_rank_{}".format(rank)
)
...
local_best_hps = tuner.get_best_hyperparameters(num_trials=1)[0]
# Gather best hyperparameters from all processes to rank 0.
all_best = comm.gather(local_best_hps.values, root=0)
if rank == 0:
    print("Best hyperparameters from each MPI process:")
    for i, best in enumerate(all_best):
        print("Process {}: {}".format(i, best))
else:
    # For non-root processes, you may optionally print your local best.
    print("Rank {} local best hyperparameters: {}".format(rank, local_best_hps.values))

# Build the best model (each process continues with its own best) and train it further.
best_model = tuner.hypermodel.build(local_best_hps)
...
best_model.save(os.path.join("models", "chain_hps_6D_trained_model_rank_{}.h5".format(rank)))
with open(os.path.join(data_dir, "chain_hps_6D_training_history_rank_{}.pkl".format(rank)), "wb") as f:
    pickle.dump(history.history, f)

# Plot training and validation loss (each process saves its own plot).
plt.figure(figsize=(8, 6))
plt.plot(history.history["loss"], label="Training Loss")
plt.plot(history.history["val_loss"], label="Validation Loss")
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.title("6D Random Search Training History (Rank {})".format(rank))
plt.legend()
plt.yscale("log")
plt.grid(True)
os.makedirs("plots", exist_ok=True)
plt.savefig(os.path.join("plots", "chain_hps_6D_training_loss_rank_{}.png".format(rank)))
plt.show()
